 ...
 * 发票分页查询响应（进项发票）
 *
 * <p>对应页面：财务管理:发票管理:进项发票:页面</p>
 * <p>字段权限：仅绑定进项发票页面的字段级权限编码</p>
 */
@Data
@ApiModel("发票分页查询响应（进项发票）")
public class InputInvoicePageResponse {
     ...
     */
    public static InputInvoicePageResponse from(InvoicePageResponse source) {
        if (source == null) {
            return null;
        }
        InputInvoicePageResponse target = new InputInvoicePageResponse();
