 ...
 * 结算单分页响应（付款结算）
 *
 * <p>对应页面：合同结算:危险废物结算-付款结算:页面</p>
 */
@Data
@ApiModel("结算单分页响应（付款结算）")
public class PayableSettlementPageResponse {
     ...
     */
    public static PayableSettlementPageResponse from(SettlementPageResponse source) {
        if (source == null) {
            return null;
        }
        PayableSettlementPageResponse target = new PayableSettlementPageResponse();
