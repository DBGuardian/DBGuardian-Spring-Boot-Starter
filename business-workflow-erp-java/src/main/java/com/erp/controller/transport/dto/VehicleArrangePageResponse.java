...
/**
 * 车辆安排列表分页行 DTO（车辆安排页面专用）
 */
@Data
@ApiModel("收运通知单分页响应-车辆安排页面")
public class VehicleArrangePageResponse {
    ...
    /**
     * 从通用 DTO 转换为车辆安排页面专用 DTO
     */
    public static VehicleArrangePageResponse from(TransportApplyPageResponse source) {
        if (source == null) {
            return null;
        }
        VehicleArrangePageResponse target = new VehicleArrangePageResponse();
