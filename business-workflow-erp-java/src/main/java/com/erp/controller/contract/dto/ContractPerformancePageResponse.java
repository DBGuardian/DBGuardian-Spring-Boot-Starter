 ...
 */
@Data
@ApiModel("合同分页响应（合同履行）")
public class ContractPerformancePageResponse {
    ...
    // 合同履行页面无“文档”字段级权限配置，但为保持响应结构一致，仍返回该字段（不受字段级权限控制）。
     ...
     */
    public static ContractPerformancePageResponse from(ContractPageResponse source) {
        if (source == null) {
            return null;
        }
        ContractPerformancePageResponse target = new ContractPerformancePageResponse();
