...
public class QuotationPdfGenerator {
     ...
     * 生成报价单PDF文件
     *
     * @param quotationDetail 报价单详情
     * @param filePath        文件保存路径
     * @throws DocumentException PDF生成异常
     * @throws IOException       IO异常
     */
    public static void generatePdf(QuotationDetailResponse quotationDetail, String filePath)
        ...
        try {
            ...
            // 添加报价单号
            addQuotationNo(document, quotationDetail.getQuotationNo());

            // 添加表格
            addTable(document, quotationDetail);

            // 添加备注（自动检查并添加服务期限信息）
            addRemark(document, quotationDetail);

            // 添加签名区域
            addSignatureArea(document, quotationDetail);
     ...
     * 添加报价单号
     */
    private static void addQuotationNo(Document document, String quotationNo) throws DocumentException {
        if (quotationNo != null && !quotationNo.trim().isEmpty()) {
            Paragraph quotationNoPara = new Paragraph("报价单号：" + quotationNo, NORMAL_FONT);
            quotationNoPara.setAlignment(Element.ALIGN_RIGHT);
            quotationNoPara.setSpacingAfter(15);
            document.add(quotationNoPara);
        }
    }
     ...
     * 添加表格
     */
    private static void addTable(Document document, QuotationDetailResponse quotationDetail) throws DocumentException {
        // 创建表格：序号、废物类别、废物代码、废物名称、废物形态、计划转移数量、金额、付款方
        ...
        if (quotationDetail.getItems() != null) {
            for (QuotationDetailResponse.QuotationItemResponse item : quotationDetail.getItems()) {
                if (item.getWasteItems() != null && !item.getWasteItems().isEmpty()) {
                    ...
                    if (isPackageMode) {
                        // 构建金额字符串：quotationItemId | pricingPlan（使用 | 作为分隔符）
                        StringBuilder amountBuilder = new StringBuilder();
                        if (item.getQuotationItemId() != null) {
                            amountBuilder.append(item.getQuotationItemId());
                        }
                        ...
                        // 构建付款方字符串：quotationItemId | payer（使用 | 作为分隔符）
                        StringBuilder payerBuilder = new StringBuilder();
                        if (item.getQuotationItemId() != null) {
                            payerBuilder.append(item.getQuotationItemId());
                        }
                    ...
                    // 每个危废条目一行，但总价包干模式下金额和付款方从报价条目读取
                    for (QuotationDetailResponse.QuotationWasteItemResponse wasteItem : item.getWasteItems()) {
                        // 保存行数据
                        ...
                        // 不再保存 quotationItem 到 rowData（未在后续处理使用）
        ...
        // 如果存在价外服务，追加单独表格显示
        addOutOfScopeServicesTable(document, quotationDetail);
     ...
     * 添加价外服务表格（在主表之后显示）
     */
    private static void addOutOfScopeServicesTable(Document document, QuotationDetailResponse quotationDetail) throws DocumentException {
        if (quotationDetail == null || quotationDetail.getOutOfScopeServices() == null || quotationDetail.getOutOfScopeServices().isEmpty()) {
            return;
        }

        // 创建价外服务表格：序号、项目、规格型号、单位、计划数量、合同单价
        PdfPTable outTable = new PdfPTable(6);
        outTable.setWidthPercentage(100);
        outTable.setSpacingBefore(6);
        outTable.setSpacingAfter(10);
        outTable.setWidths(new float[]{1f, 2.5f, 2f, 01f, 2f, 2f});
        ...
        // 内容行
        int idx = 1;
        for (QuotationDetailResponse.OutOfScopeServiceResponse svc : quotationDetail.getOutOfScopeServices()) {
            // 序号
     ...
     * @param quotationItem 报价条目（总价包干模式时传入，用于读取计价方案和付款方）
     * @param combinedAmountStr 组合后的金额字符串（总价包干模式：quotationItemId + pricingPlan）
     * @param combinedPayerStr 组合后的付款方字符串（总价包干模式：quotationItemId + payer）
     */
    ...
    private static void addSubtotalSummaryRows(PdfPTable table, String subtotalSummaryJson) throws DocumentException {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                // 如果 total == -1，表示该 unit 为不限量，标记为 "序号N不限量"（若 unit 存在则使用 unit + "不限量"）
            ...
            for (String it : subtotalItems) {
                if (it != null && it.contains("不限量")) {
                    ...
                    // regular item, preserve order and duplicates
                    deduped.add(it);
                }
            }
    ...
    private static String calculateTotalFromSubtotalSummary(List<String> allSubtotalSummaryList) {
        ...
        try {
            ...
            for (String subtotalSummaryJson : allSubtotalSummaryList) {
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    ...
                    // total == -1 表示该 unit 为不限量（标记，不进行数值累加）
        ...
        // 如果没有任何已知unit（全部被过滤或为空），返回不限量
        if (unitOrder.isEmpty()) {
            return "不限量";
        }

        // 组装每个unit的显示文本：若该unit被标记为不限量，显示 "不限量"；否则显示累计值 + 单位
     ...
     * 添加备注（自动检查并添加服务期限信息）
     */
    private static void addRemark(Document document, QuotationDetailResponse quotationDetail) throws DocumentException {
        String remark = quotationDetail.getRemark();
        if (remark == null) {
            remark = "";
        }
        remark = remark.trim();
        ...
        // 构建服务期限信息
        String servicePeriod = buildServicePeriod(quotationDetail);
     ...
     */
    private static String buildServicePeriod(QuotationDetailResponse quotationDetail) {
        if (quotationDetail.getValidFrom() == null || quotationDetail.getValidTo() == null) {
            return null;
        }
        
        // 格式化日期：yyyy年MM月dd日
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy年M月d日");
        String validFromStr = quotationDetail.getValidFrom().format(formatter);
        String validToStr = quotationDetail.getValidTo().format(formatter);
     ...
     * 添加签名区域
     */
    private static void addSignatureArea(Document document, QuotationDetailResponse quotationDetail)
        ...
        // 甲方
        PdfPCell partyACell = createSignatureCell("甲方(盖章)", quotationDetail.getPartyAName(),
                quotationDetail.getPartyAContact(), quotationDetail.getPartyAContactPhone());
        signatureTable.addCell(partyACell);

        // 乙方
        PdfPCell partyBCell = createSignatureCell("乙方(盖章)", quotationDetail.getPartyBName(),
                quotationDetail.getPartyBContact(), quotationDetail.getPartyBContactPhone());
     ...
     * 创建签名单元格
     */
    private static PdfPCell createSignatureCell(String title, String name, String contact, String phone) {
        ...
        if (name != null && !name.trim().isEmpty()) {
            PdfPCell nameCell = new PdfPCell(new Phrase("单位名称：" + name, NORMAL_FONT));
        ...
        if (contact != null && !contact.trim().isEmpty()) {
            PdfPCell contactCell = new PdfPCell(new Phrase("业务联系人：" + contact, NORMAL_FONT));
            contactCell.setBorder(Rectangle.NO_BORDER);
            contactCell.setPaddingBottom(5);
            innerTable.addCell(contactCell);
        }

        // 联系电话
        if (phone != null && !phone.trim().isEmpty()) {
            PdfPCell phoneCell = new PdfPCell(new Phrase("联系电话：" + phone, NORMAL_FONT));
    ...
    private static class TableRowData {
        ...
        private QuotationDetailResponse.QuotationWasteItemResponse wasteItem;
        private String quotationMode;
            // quotationItem removed (not used in processing)
        ...
        public QuotationDetailResponse.QuotationWasteItemResponse getWasteItem() { return wasteItem; }
        public void setWasteItem(QuotationDetailResponse.QuotationWasteItemResponse wasteItem) { this.wasteItem = wasteItem; }
        ...
        // setQuotationItem removed (not used)
    ...
    private static void addTableRowsWithMerge(PdfPTable table, List<TableRowData> rowDataList, 
        ...
        for (TableRowData rowData : rowDataList) {
            ...
            if (isPackageMode && rowData.getAmountStr() != null && !rowData.getAmountStr().trim().isEmpty()) {
                // 总价包干模式：使用传入的组合字符串，并检查是否需要添加超量单价
                String baseAmountStr = rowData.getAmountStr();
                String parentPricingPlan = removeQuotationItemId(baseAmountStr);
                
                // 总价包干模式下：总是显示父节点（quotationItem 的计价方案），
                // 并根据是否启用辅助核算补充显示 基础单价、辅助单价 等列（每个一行）
                StringBuilder pkgAmountBuilder = new StringBuilder();
                if (StrUtil.isNotBlank(parentPricingPlan)) {
                    pkgAmountBuilder.append(parentPricingPlan);
                }

                // 获取当前危废条目，依据其字段补充显示基础单价 / 辅助单价
                QuotationDetailResponse.QuotationWasteItemResponse wi = rowData.getWasteItem();
                boolean auxEnabled = Boolean.TRUE.equals(wi.getEnableAuxiliaryAccounting());

                // 当启用辅助核算时，显示：父节点计价方案 / 基础单价（元/吨） / 辅助单价（元/auxUnit）
                // 当未启用辅助核算时，显示：父节点计价方案 / 基础单价（元/吨）
                if (auxEnabled) {
                    // 始终显示三行：父节点计价方案 / 基础单价（或占位） / 辅助单价（或占位）
                    ...
                    if (wi.getBaseUnitPrice() != null) {
                        pkgAmountBuilder.append(wi.getBaseUnitPrice().stripTrailingZeros().toPlainString())
                                .append("元/吨");
                    } else if(wi.getPricingPlan() != null && !wi.getPricingPlan().trim().isEmpty()) {
                        pkgAmountBuilder.append(wi.getPricingPlan()).append("元/吨");
                    ...
                    if (wi.getAuxUnitPrice() != null) {
                        pkgAmountBuilder.append(wi.getAuxUnitPrice().stripTrailingZeros().toPlainString())
                                .append("元/");
                        pkgAmountBuilder.append(StrUtil.isNotBlank(wi.getAuxUnit()) ? wi.getAuxUnit() : "-");
                    } else {
                        pkgAmountBuilder.append("-").append("元/").append(StrUtil.isNotBlank(wi.getAuxUnit()) ? wi.getAuxUnit() : "-");
                    }
                } else {
                    // 未启用辅助核算：显示父节点 + 基础单价（若无则用基础数量或占位）
                    pkgAmountBuilder.append("\n");
                    if (wi.getBaseUnitPrice() != null) {
                        pkgAmountBuilder.append(wi.getBaseUnitPrice().stripTrailingZeros().toPlainString())
                                .append("元/吨");
                ...
                amountStr = pkgAmountBuilder.length() > 0 ? pkgAmountBuilder.toString() : parentPricingPlan;
            } else if (!isPackageMode && rowData.getWasteItem().getPricingPlan() != null && !rowData.getWasteItem().getPricingPlan().trim().isEmpty()) {
                // 按量结算模式：需要同时展示计价方案（作为基础单价表达）与辅助单价（如果启用辅助核算）
                String pricingPlan = rowData.getWasteItem().getPricingPlan().trim();
                StringBuilder amountBuilder = new StringBuilder();

                // 1) 计价方案行：基础计量单位固定为 吨，计价单位固定为 元/吨
                if (StrUtil.isNotBlank(pricingPlan)) {
                    amountBuilder.append(pricingPlan).append("元/吨");
                }

                // 2) 如果启用了辅助核算且存在辅助单价，则追加辅助单价（元/辅助单位）
                if (Boolean.TRUE.equals(rowData.getWasteItem().getEnableAuxiliaryAccounting())
                        && rowData.getWasteItem().getAuxUnitPrice() != null
                        && StrUtil.isNotBlank(rowData.getWasteItem().getAuxUnit())) {
                    if (amountBuilder.length() > 0) {
                        amountBuilder.append("\n");
                    }
                    String auxPriceStr = rowData.getWasteItem().getAuxUnitPrice()
                            .stripTrailingZeros()
                            .toPlainString();
                    amountBuilder.append(auxPriceStr)
                            .append("元/")
                            .append(rowData.getWasteItem().getAuxUnit());
                } else if (rowData.getWasteItem().getBaseUnitPrice() != null) {
                    // 若存在基础单价，则在没有 aux 单价时补充显示基础单价，单位固定为 元/吨
                    if (amountBuilder.length() == 0) {
                        amountBuilder.append(rowData.getWasteItem().getBaseUnitPrice().stripTrailingZeros().toPlainString())
                                .append("元/吨");
                    }
                }

                amountStr = amountBuilder.length() > 0 ? amountBuilder.toString() : pricingPlan;
            } else if (rowData.getAmountStr() != null && !rowData.getAmountStr().trim().isEmpty()) {
                // 其他情况，使用传入的金额字符串
                amountStr = removeQuotationItemId(rowData.getAmountStr());
            }
        ...
        for (int i = 0; i < rowDataList.size(); i++) {
            ...
            if (rowData.getWasteItem().getPlannedQuantity() != null) {
                ...
                if (planned.compareTo(BigDecimal.valueOf(-1)) == 0) {
                    ...
                    // 启用了辅助核算且非无限量：显示两行，第一行为基础（吨），第二行为辅助（auxUnit）
                    String baseQtyStr = "-";
                    ...
                    } else if (rowData.getWasteItem().getPlannedQuantity() != null) {
                        baseQtyStr = rowData.getWasteItem().getPlannedQuantity().stripTrailingZeros().toPlainString() + " 吨";
                    }
                    String auxQtyStr = "-";
                    ...
                    // 未启用辅助核算：常规展示计划数量 + 单位（优先 unit，其次近似基础数量并固定单位为 吨）
            ...
            if (payerStr != null) {
                ...
                if (payerRowspan[i] > 1) {
                    // 需要合并，去掉 quotationItemId，只显示 payer
                    String mergedPayerStr = removeQuotationItemId(payerStr);
                    ...
                    // 单行数据
                    if (isPackagePayer) {
                        // 总价包干模式，即使只有一条数据也要去掉 quotationItemId
                        String displayPayerStr = removeQuotationItemId(payerStr);
    ...
    /**
     * 从组合字符串中移除 quotationItemId，只保留后面的内容
     * 例如："18|1500元/吨" -> "1500元/吨"
     *      "20|甲方" -> "甲方"
     * 
     * @param combinedStr 组合字符串（格式：quotationItemId | 内容，使用 | 作为分隔符）
     * @return 去掉 quotationItemId 后的字符串
     */
    private static String removeQuotationItemId(String combinedStr) {
