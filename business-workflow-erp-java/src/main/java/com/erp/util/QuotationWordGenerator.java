...
@Slf4j
public class QuotationWordGenerator {

    private QuotationWordGenerator() {
    }

    public static void generateWord(QuotationDetailResponse quotationDetail, OutputStream outputStream) throws IOException {
        try {
            ...
            createBasicInfoSection(documentPart, quotationDetail);
            createDetailTable(documentPart, quotationDetail);
            Docx4jHelper.addBlankLines(documentPart, 1);
            createOutOfScopeServicesTable(documentPart, quotationDetail);
            createRemarkSection(documentPart, quotationDetail);
            createSignatureSection(documentPart, quotationDetail);
    ...
    private static void createBasicInfoSection(MainDocumentPart documentPart, QuotationDetailResponse detail) {
        P paragraph = Docx4jHelper.createParagraph(documentPart);
        Docx4jHelper.setAlignment(paragraph, "LEFT");
        R run = Docx4jHelper.createRun(paragraph);
        Docx4jHelper.setFontSize(run, 11);
        appendLabelValue(run, "报价单号：", detail.getQuotationNo());
    }
    ...
    private static void createDetailTable(MainDocumentPart documentPart, QuotationDetailResponse detail) {
        ...
        for (RowData rowData : rows) {
            ...
            if (isSubtotalOrTotal) {
                ...
                Docx4jHelper.setCellHorizontalAlignment(cell2, "LEFT");
    ...
    private static List<RowData> buildRowData(QuotationDetailResponse detail) {
        List<RowData> rows = new ArrayList<>();
        List<String> subtotalJsonList = new ArrayList<>();
        int index = 1;
        if (detail.getItems() != null) {
            for (QuotationDetailResponse.QuotationItemResponse item : detail.getItems()) {
                ...
                for (QuotationDetailResponse.QuotationWasteItemResponse waste : item.getWasteItems()) {
    ...
    private static String buildQuantityCell(QuotationDetailResponse.QuotationWasteItemResponse waste) {
    ...
    private static String buildAmountCell(QuotationDetailResponse.QuotationWasteItemResponse waste) {
    ...
    private static String buildPackageAmountCell(String parentPricingPlan, QuotationDetailResponse.QuotationWasteItemResponse waste) {
    ...
    private static void createRemarkSection(MainDocumentPart documentPart, QuotationDetailResponse detail) {
    ...
    private static String buildRemarkContent(QuotationDetailResponse detail) {
    ...
    private static String buildServicePeriod(QuotationDetailResponse detail) {
    ...
    private static void createOutOfScopeServicesTable(MainDocumentPart documentPart, QuotationDetailResponse detail) {
        ...
        int[] widthRatios = {2, 5, 4, 2, 4, 4};
        Docx4jHelper.setColumnWidthsByRatios(table, widthRatios, documentPart);

        int idx = 1;
        for (QuotationDetailResponse.OutOfScopeServiceResponse svc : detail.getOutOfScopeServices()) {
            Tr row = Docx4jHelper.createTableRow(table);
    ...
    private static void createSignatureSection(MainDocumentPart documentPart, QuotationDetailResponse detail) {
        Tbl table = Docx4jHelper.createTable(documentPart);
        ...
        fillSignatureCell(cellA, "甲方(盖章)", detail.getPartyAName(),
                detail.getPartyAContact(), detail.getPartyAContactPhone());

        fillSignatureCell(cellB, "乙方(盖章)", detail.getPartyBName(),
                detail.getPartyBContact(), detail.getPartyBContactPhone());
    }

    private static void fillSignatureCell(Tc cell, String title, String name, String contact, String phone) {
        ...
        if (StrUtil.isNotBlank(contact)) {
            content.append("\n业务联系人：").append(contact);
        }
        if (StrUtil.isNotBlank(phone)) {
            content.append("\n联系电话：").append(phone);
        }
        Docx4jHelper.setCellText(cell, content.toString());
    }
